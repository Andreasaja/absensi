...
<title>REPORT DAFTAR SLIP GAJI PERIODIKAL</title>
</head>
<body>
<?php
    if (isset($fperiode))
        ...
        else { $qslipgaji = $connection->query("select * from slipgaji where periode = '$fperiode' order by nama asc;"); }
        }
    else
        { $qslipgaji = $connection->query("select * from slipgaji order by idxsg desc limit 150;"); }
    if (mysqli_num_rows($qslipgaji) > 0)
        ...
        while ($aqslipgaji = mysqli_fetch_array($qslipgaji))
            ...
            echo "<tr><td>Hari Kerja</td><td>:</td><td align='right'>".$thrkerja."</td></tr>";
            echo "<tr><td>Lembur 1</td><td>:</td><td align='right'>".$tlembur1."</td></tr>";
            echo "<tr><td>Lembur 2</td><td>:</td><td align='right'>".$tlembur2."</td></tr>";
            echo "</table></td>";
